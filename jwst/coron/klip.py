...
def klip(target_model, refs_model, truncate):
    ...
    refs_model : ModelLibrary, (NTARG) models each of shape (NINTS x NROWS x NCOLS)
    ...
    with refs_model:
        for i in range(target_model.data.shape[0]):
            ...
            refs = refs_model.borrow(i).data.astype(np.float64)
            rshape = refs.shape
            nrefs = rshape[0]
            ...
            refs_model.shelve(refs, i, modify = False)
        del refs_fit, refs, klvect, eigval, eigvect
